				...
				case "embed":
					...
					case "color":
						...
						if strings.HasPrefix(ctx.Args[2], "#") {
							...
							if err != nil {
								ctx.DB.Log("Config", ctx.Guild.ID, fmt.Sprintf("error setting parameter %v to value %v: %v", ctx.Args[1], target[2], err.Error()))
								return
							}
						} else {
							color, err = strconv.ParseInt(ctx.Args[2], 16, 32)
							if err != nil {
								ctx.DB.Log("Config", ctx.Guild.ID, fmt.Sprintf("error setting parameter %v to value %v: %v", ctx.Args[1], ctx.Args[2], err.Error()))
								return
							}
						}
